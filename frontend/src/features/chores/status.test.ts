...
describe('getChoreDueStatus', () => {
  const now = new Date('2026-07-15T00:00:00Z')

  it('marks a task due within 24 hours', () => {
    expect(getChoreDueStatus(makeTask(), now)).toEqual({ state: 'due-soon', label: '24時間以内' })
  })

  it('marks an overdue task', () => {
    const task = makeTask({ next_due_at: '2026-07-13T00:00:00Z' })

    expect(getChoreDueStatus(task, now)).toEqual({ state: 'overdue', label: '2日超過' })
  })
...
describe('getChoreProgress', () => {
  ...
  it('reports elapsed time and a healthy color state', () => {
    ...
    expect(getChoreProgress(task, new Date('2026-07-12T00:00:00Z'))).toEqual({
      state: 'scheduled',
      percent: 40,
      elapsedDays: 2,
      remainingDays: 3,
  ...
  it('switches to the warning state when 20 percent remains', () => {
    ...
    expect(getChoreProgress(task, new Date('2026-07-14T00:00:00Z'))).toMatchObject({
      state: 'due-soon',
      percent: 80,
      elapsedDays: 4,
      remainingDays: 1,
  ...
  it('uses the overdue state at the deadline and clamps later progress to 100 percent', () => {
    ...
    expect(getChoreProgress(task, new Date('2026-07-15T00:00:00Z'))).toMatchObject({
      state: 'overdue',
      percent: 100,
      elapsedDays: 5,
      remainingDays: 0,
    })
    expect(getChoreProgress(task, new Date('2026-07-16T00:00:00Z'))).toMatchObject({
      state: 'overdue',
      percent: 100,
      elapsedDays: 5,
      remainingDays: 0,
  ...
  it('returns a neutral state for inactive tasks', () => {
    expect(getChoreProgress(makeTask({ is_active: false }), now)).toEqual({
      state: 'inactive',
      percent: 0,
      elapsedDays: 0,
      remainingDays: 1,
