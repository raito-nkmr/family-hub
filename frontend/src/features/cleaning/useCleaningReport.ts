...
import { getCleaningMonthlyReport } from './api'
import { getCurrentMonthForTimezone, isReportMonth } from './reportDate'

interface UseCleaningReportOptions {
  onUnauthorized: () => void
}

export function useCleaningReport({ onUnauthorized }: UseCleaningReportOptions) {
  ...
  const reportQuery = useQuery({
    queryKey: queryKeys.cleaningReport(selectedGroupId ?? '', effectiveMonth),
    queryFn: ({ signal }) => getCleaningMonthlyReport(selectedGroupId!, effectiveMonth, signal),
  ...
  const queryError = groupsQuery.error
    ? i18n.t('errors.cleaningReportData')
    : reportQuery.error
      ? isApiErrorWithStatus(reportQuery.error, 404)
        ? i18n.t('errors.cleaningReportForbidden')
        : i18n.t('errors.cleaningReportLoad')
      : null
