...
test('navigates the authenticated household apps on iPhone WebKit', async ({ page }) => {
  ...
  await expect(page.getByRole('heading', { level: 1, name: 'Shopping now', exact: true })).toBeVisible()
  await expect(page.getByRole('heading', { name: 'A family group is required', exact: true })).toBeVisible()
