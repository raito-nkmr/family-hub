...
ホーム、写真、アルバム、家族グループ、掃除管理、買い物リスト、およびアカウント安全設定を提供する
...
│   ├── cleaning/   # 家族グループ単位の掃除箇所と完了管理
│   ├── groups/     # 家族グループAPI、一覧、メンバー管理
│   ├── home/       # 写真、掃除、および買い物を横断するホーム画面
...
キャッシュキーは`src/shared/api/queryKeys.ts`へ集約します。ホーム、アルバム、グループ、買い物、掃除、招待、通知、
...
開くと、現在の端末で通知を有効化・解除し、写真共有、掃除期限、および買い物追加の通知設定を変更できます。iPhoneでは
