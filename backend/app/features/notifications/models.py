...
class PushSubscription(Base):
    ...
    __table_args__ = (
        ...
        ForeignKeyConstraint(
            ...
            ["user_sessions.id", "user_sessions.user_id"],
            name="fk_push_subscriptions_user_session_user_id_user_sessions",
            ondelete="CASCADE",
        ),
...
class NotificationOutbox(Base):
    ...
    __table_args__ = (
        ...
        CheckConstraint(
            ...
            "(status = 'processing' AND processed_at IS NULL AND claimed_at IS NOT NULL "
            "AND claim_token IS NOT NULL) OR "
            "(status IN ('sent', 'failed') AND processed_at IS NOT NULL AND claimed_at IS NULL "
...
Index(
    ...
    postgresql_where=NotificationOutbox.status == "pending",
)
...
class NotificationDelivery(Base):
    ...
    __table_args__ = (
        ...
        CheckConstraint(
            ...
            "(status IN ('sent', 'failed') AND processed_at IS NOT NULL)",
...
Index("ix_notification_deliveries_subscription_id", NotificationDelivery.subscription_id)
